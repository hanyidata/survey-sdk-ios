...
class ViewController: UIViewController {
    var _survey:SurveyViewController? = nil
    ...
    @IBAction func clickShow(_ sender: Any) {
        ...
        if ((_survey == nil)) {
            self._survey = SurveyViewController.makeSurveyController(surveyId: sid, channelId: channel, parameters: ["externalUserId": euid], options: ["delay": 1000, "debug": true])
            
            addChildViewController(self._survey!)
            let newFrame = CGRectMake( 0, 300, self.view.frame.width, 0)
            self._survey!.view.frame = newFrame
            view.addSubview(self._survey!.view)
